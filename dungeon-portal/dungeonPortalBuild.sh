#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"

UP=false

PORTAL_SERVICES=(postgres dungeon-portal-api dungeonportal-frontend)

usage() {
  echo "Uso: $0 [--up]"
  echo "  --up   dopo la build, avvia solo i servizi del portale (${PORTAL_SERVICES[*]})"
  echo
  echo "Builda sempre senza cache Docker (--no-cache) e, a fine build,"
  echo "elimina le immagini e la build cache non piu' utilizzate."
  echo
  echo "Non tocca altri servizi eventualmente definiti nello stesso docker-compose.yml"
  echo "(es. istanze FoundryVTT) - costruisce e avvia solo il portale."
}

for arg in "$@"; do
  case "$arg" in
    --up) UP=true ;;
    -h|--help) usage; exit 0 ;;
    *) echo "Argomento non riconosciuto: $arg" >&2; usage; exit 1 ;;
  esac
done

echo "==> Build immagini Docker (api + frontend), senza cache"
docker compose build --no-cache dungeon-portal-api dungeonportal-frontend

if [ "$UP" = true ]; then
  echo "==> Avvio servizi del portale: ${PORTAL_SERVICES[*]}"
  docker compose up -d "${PORTAL_SERVICES[@]}"
fi

echo "==> Pulizia immagini non utilizzate (dangling)"
docker image prune -f

echo "==> Pulizia build cache non utilizzata"
docker builder prune -f

echo "==> Fatto."
