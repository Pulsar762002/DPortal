...
3. `StoryBlock` (`core/models/story-block.model.ts`) is a discriminated union on `type`: `paragraph`, `quote`, `image` (variant `left`/`left-small`/`left-big`/`center`), `scene` (full-width establishing shot with overlaid title), `divider` (ornamental separator) and `note` (Master's aside). The rendering happens inline in `dynamic-session.component.html` (a `*ngIf` chain over `block.type`): `paragraph`/`quote`/`image` are plain elements, while `scene`/`divider`/`note` are standalone components in `shared/story-blocks/`. **To add a new block type: add it to the union, create its component there, and add one `*ngIf` branch in `dynamic-session.component.html`** — that template is the single source of truth (there is no separate dispatcher).
