...
3. `StoryBlock` (`core/models/story-block.model.ts`) is a discriminated union on `type`: `paragraph`, `quote`, `image` (image has a `variant` controlling layout: `left`/`left-small`/`left-big`/`center`). Components live in `shared/story-blocks/`.
